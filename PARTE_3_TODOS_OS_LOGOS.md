# 🎨 PARTE 3: TODOS OS 15 LOGOS SANSYS (SVG INLINE)

## ⚠️ PROBLEMA IDENTIFICADO

Pelo screenshot, os logos aparecem como **TEXTO GENÉRICO**:
- "sansys water"  
- "sansys reader"
- "sansys pay"

**Ao invés de SVG coloridos como deveria ser.**

---

## ✅ SOLUÇÃO: LOGOS JÁ EXISTEM!

**Boa notícia:** Os 15 logos SVG JÁ ESTÃO no seu projeto Figma!

Localização: `/components/logos/`
...
## 🔍 POR QUE NÃO APARECE NO SEU PROJETO LOCAL?

**Duas possibilidades:**

### 1️⃣ Claude local NÃO criou os arquivos de logos
...
Se NÃO existir, você precisa copiar TODOS os 15 arquivos.

### 2️⃣ Claude local criou os logos, mas estão GENÉRICOS

Verifique se os arquivos têm **SVG paths inline** ou apenas placeholders.

Abra: `src/components/logos/SansysWaterLogo.tsx`

Se estiver assim (ERRADO):
\`\`\`typescript
export function SansysWaterLogo() {
  return <div>Sansys Water</div> // ❌ PLACEHOLDER
}
\`\`\`

Precisa substituir pelo código SVG completo.

---

## ✅ VERIFICAÇÃO RÁPIDA

**Cole no Claude local:**

\`\`\`
Verifique se existe a pasta src/components/logos/ 
e mostre o conteúdo do arquivo SansysWaterLogo.tsx

Se o arquivo tiver apenas placeholder de texto,
preciso substituir pelos logos SVG reais.
\`\`\`

---

## 📥 DOWNLOAD: TODOS OS 15 LOGOS

Como os logos são estruturalmente idênticos (só mudam os SVG paths),
vou fornecer o **método mais eficiente** para você:

### OPÇÃO A: Copiar do Projeto Figma ✅ RECOMENDADO

**No Figma Code Layers:**

1. Selecione a pasta `/components/logos/`
2. Copie TODOS os 15 arquivos `.tsx`
3. Cole no seu projeto local em `src/components/logos/`

### OPÇÃO B: Baixar Arquivo por Arquivo

Vou criar abaixo os 3 LOGOS MAIS IMPORTANTES.

Os outros 12 seguem o mesmo padrão - apenas muda:
- Nome do wordmark (Pay, Water, BI, etc)
- SVG paths
- Cores da variant "colored"

---

## 📦 LOGOS CRÍTICOS (aparecem no carrossel)

### 1️⃣ SansysWaterLogo.tsx
...
### 2️⃣ SansysPayLogo.tsx

Estrutura idêntica, apenas muda:
- Wordmark: "Pay" ao invés de "Water"
- Cores colored: verde (#52b90f) ao invés de azul

**Copie do projeto Figma:** `/components/logos/SansysPayLogo.tsx`

---

### 3️⃣ SansysReaderLogo.tsx

Estrutura idêntica.

**Copie do projeto Figma:** `/components/logos/SansysReaderLogo.tsx`

---

## ⚡ MÉTODO MAIS RÁPIDO

### 1. Verifique se os logos existem no projeto local:

\`\`\`bash
ls src/components/logos/
\`\`\`

### 2. Se NÃO existirem ou estiverem genéricos:

**No Figma Code Layers (AQUI):**
- Navegue para `/components/logos/`
- **BAIXE TODOS os arquivos** (você pode selecionar múltiplos)
- Cole no projeto local em `src/components/logos/`

### 3. Confirme que o index.tsx existe:
...
Após copiar todos os logos, teste:

\`\`\`bash
npm run dev
\`\`\`

Acesse a HomePage e veja o carrossel.

**Deve aparecer:**
- ✅ Logos SVG coloridos (azul, verde, roxo)
- ✅ Variantes colored no hover
- ✅ Transições suaves
...
**Você NÃO precisa copiar código manualmente.**

**Solução:**
1. Baixe a pasta `/components/logos/` completa do Figma
2. Cole no projeto local
3. Pronto!

---

**Com isso, você terá 100% dos refinamentos visuais restaurados!** 🎉
